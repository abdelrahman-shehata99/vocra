...
### Added
- Swift Package Manager support on iOS: `ios/vocra_flutter/Package.swift`, with the
  native AEC sources moved to `ios/vocra_flutter/Sources/vocra_flutter/`. The
  CocoaPods podspec is still maintained, so apps that have not enabled SPM build
  unchanged.

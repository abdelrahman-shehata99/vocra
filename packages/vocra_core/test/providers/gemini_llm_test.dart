...
void main() {
  group('GeminiLlm', () {
    test('streams text parsed from candidates.content.parts fixture', () async {
    ...
    test('maps history to Gemini format: system -> systemInstruction, '
        'assistant -> model role, leading assistant dropped', () async {
      final adapter = FakeHttpClientAdapter((options) async {
        expect(
          options.path,
    ...
    test('a SAFETY finish surfaces as a typed ProviderError', () async {
      ...
      await expectLater(
        stream,
        emitsInOrder(['so far so good', emitsError(isA<ProviderError>())]),
      );
    });
  });
}

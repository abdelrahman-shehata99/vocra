...
- State machine discipline: `TurnMachine` (packages/voice_core/lib/src/engine/turn_machine.dart)
...
- Tests: `test` package for voice_core (mocktail for mocking, `stream_channel` /
  `fake_websocket_channel.dart` fakes for WS-based providers), `flutter_test` for voice_flutter.

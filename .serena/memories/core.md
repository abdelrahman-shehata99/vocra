...
`mem:vocra_flutter/core` (Flutter plugin layer, depends on vocra_core).

Root layout:
- `packages/vocra_core/` — engine, provider adapters, transport (see `mem:vocra_core/core`)
- `packages/vocra_flutter/` — mic/audio/permissions/VoiceSession + `example/` demo app
...
- `vocra_core` must never import `package:flutter` — providers/engine are pure Dart, unit
  tested with `dart test`; `vocra_flutter` is the only package tested with `flutter test`.

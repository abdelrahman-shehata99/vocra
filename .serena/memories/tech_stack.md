...
- `voice_core`: pure Dart, deps `dio` (HTTP), `web_socket_channel` (WS). Dev deps: `test`,
  `mocktail`, `lints`.
- `voice_flutter`: Flutter plugin, deps `voice_core` (path dep), `just_audio`, `record`,
...
- `packages/voice_flutter/example/` is a runnable Flutter demo app + manual test harness
